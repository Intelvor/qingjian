# 青简 Qingjian · Windows 增强版

> 输入的不只是文字。

**这是[青简（qingjian-team/qingjian）](https://github.com/qingjian-team/qingjian)的一个 Windows 增强版**：
在上游基础上针对 Windows 平台补齐了一批交互与设置项，其余部分（词库、Core、macOS 壳）与上游保持同步。

相比上游，Windows 这一路多出来的大致是：

- **候选窗能用鼠标**：候选行点选上屏、悬停浅底色，整句补全那块也能点。命中矩形由渲染器给出（看得见高亮的范围就是点得着的范围）。
- **整句联想的时机可配**：`[predict] sentence_trigger = "tab"` 时停止输入不再自动请整句，按 `Tab` 才现请；等待期间候选窗摆 `☁ …`，再按一次上屏。
- **状态条**：☁ 格切换在线联想、⚙ 格打开设置；两个图标改成自绘（`☁`/`⚙` 这两个码位会被 Segoe UI Emoji 接走画成彩色，不吃我们的主题色）；中英切换键可配（单击 Shift / 单击 Ctrl / Ctrl+Space / 不切换），设置由 Server 经协议下发——DLL 不读配置文件，AppContainer 里的商店应用也能拿到同一份。
- **几处细节**：`-` `=` 由壳自己插入（放行的键在部分宿主里到不了应用）；中文模式下 Shift+字母可选「进组句」，`Cpan` 与 `cpan` 一样出「C盘」；候选词间距松开一档；用户词的词频提到 10000，刚学的词第二次就打得出来；DLL 自拉 Server 只在普通桌面应用里拉（按宿主进程的完整性级别判断）。
- **整句里的数字由模型按中文习惯取舍**：金额、数量写阿拉伯数字，口语量词写汉字——敲 `wohuayibaiershisanyuanmailesangempsan` 补出来是「我花123元买了三个mp3」。
