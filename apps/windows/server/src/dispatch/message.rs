...
impl Router {
    pub(super) fn dispatch(&mut self, message: ClientMessage) -> Option<ServerMessage> {
        match message {
            ClientMessage::OpenSession {
                ...
                // 老 DLL 的 `open` 是只写不读，多回一条会被它当成下一次 `Poll` 的应答而报错，
                // 那条连接就废了（老 DLL 在没重启的应用里还会活很久）。老 DLL 从 `ModeSync` 那一拍
                // 也能拿到同一份（新字段它直接忽略），只是慢一拍。
                (protocol >= PROTOCOL_VERSION).then(|| ServerMessage::SessionOpened {
                    session,
            ...
            ClientMessage::ModeChanged {
                ...
                // 只采纳前台应用的上报：每个加载了输入法的应用都在轮询，后台上报会覆盖状态条
                // （中英横跳、在状态条上切了又被改回去）。前台由 DLL 判断（前台窗口在本进程 /
                // 是本进程的子孙），这里照它给的结果筛。
                if !background {
            ...
            ClientMessage::SyncMode {
                ...
                session,
                // 后台应用不该取走「待切模式」：谁先到谁拿走的话，用户在悬浮状态条上点的切换会
                // 落到后台应用头上，界面又自己变回去。前台由 DLL 判断（前台窗口在本进程 / 是它的子孙）。
                english: (!background).then(|| self.take_pending_mode()).flatten(),
                input: self.input_settings(),
            }),
    ...
    fn handle_key(&mut self, session: SessionId, event: KeyEvent) -> ServerMessage {
        ...
        let (commit, outcome) = match self.apply_key(&event) {
            ...
            Effect::Navigated | Effect::Waiting => (None, KeyOutcome::Consumed),
            Effect::Passthrough => (None, KeyOutcome::Passthrough),
        };
        // 点选的词还没被 DLL 取走就又敲了个键：并进这次的上屏文本，顺序也是点选在前。
        // 放行的键 DLL 不碰文档，带了也丢，所以留着等下一次轮询。
