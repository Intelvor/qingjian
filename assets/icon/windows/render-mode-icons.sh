#!/usr/bin/env bash
# 把任务栏中 / 英 / A / 注图标的 SVG 栅格化成四档 DPI 的 8 位 alpha 蒙版，DLL 用 include_bytes! 嵌入，运行时按主题填色。
#   assets/icon/windows/render-mode-icons.sh      # 需要 rsvg-convert 与 magick
# 「注」（zhuyin）的源文件 mode-zhuyin.svg 是 render-mode-icons.ps1 从字体轮廓生成的，没有 rsvg 的机器用那个脚本。
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
OUT="$ROOT/apps/windows/tsf/resources/mode"
mkdir -p "$OUT"
for name in zh en caps zhuyin; do
