...
font = ""
# 候选窗口字体大小（点），只对青简渲染器生效；范围 8–32，超出会被夹到边界
font_size = 16
# 组句中的拼音显示在哪：both 行内和候选窗口 / inline 只在行内 / window 只在候选窗口（应用里不放 marked text）
preedit = "both"
# 英文模式（Caps Lock 亮着）是否给英文候选：Tab 或方向键选词，空格、回车、标点仍原样上屏敲的字母；
# false 就是纯直通 —— 英文模式下一个键都不吃，字母 / 数字 / 标点原样进应用（打游戏那种），中英切换键照常
...
sentence = true
# 整句补全什么时候要：idle 停止输入后自动联想（与云端词同一趟请求）；tab 只在按 Tab 键 / 点整句那块时现请一次。
# 云端词两种模式下都照常自动联想，不受这项影响
sentence_trigger = "idle"

[status_bar]
# 可拖动的悬浮状态条（Windows）：「中 / 英」格点一下切换模式（开着双拼时还显示方案名）、「，。」格切全角 / 半角标点、齿轮打开设置。
# 跟着前台应用走：显示的就是前台应用当前的中英模式，前台应用没在用青简时收起；与任务栏的中 / 英指示器并存
