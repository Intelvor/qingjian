...
pub enum ClientMessage {
    ...
    ModeChanged {
        ...
        /// 发这一条时本应用在后台（前台窗口不属于自己）。轮询那一拍会带上它：每个加载了输入法的
        /// 应用都在轮询，后台那几个的模式上报会覆盖状态条（表现是中英来回横跳），Server 只采纳前台的。
        /// 获焦 / 切换模式那两条天然是前台。老 DLL 不带，读成 false（与以前的行为一致）。
        #[serde(default)]
        background: bool,
    },
    ...
    SyncMode {
        ...
        /// 发这一条时本应用在后台（前台窗口不属于自己）。后台应用的轮询不该取走「待切模式」——
        /// 谁先到谁拿走的话，用户在悬浮状态条上点的切换会落到后台应用头上，界面又自己变回去。
        /// 老 DLL 不带，读成 false（与以前「谁问给谁」一致）。
        #[serde(default)]
        background: bool,
    },
