...
任务栏那个模式图标（语言栏按钮）仍是 Caps > 英 > 注 > 中（`com/mode/button.rs::glyph_for`）：它是**单张 16px 位图**，一格放不下「拼五」两个字，所以没跟状态条一起改成字母组合。注音开关（`[general] zhuyin`）经 `InputSettings.zhuyin` 下发，DLL 那边只在 `apply_mode_settings` 里比对「变没变」，变了就 `ModeState::notify()` 让系统重取图标（不 notify 就要等到下次切模式才刷新）。
